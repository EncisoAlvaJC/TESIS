...
La forma más natural de definir un espectro de potencias para un proceso débilmente estacionario 
...
%Partiendo de una realización de \xt, $\left\{ x(t) \right\}_{t \in T}$, se define
%\begin{equation*}
%
%\end{equation*}
...
%Para procesos en los reales $R$ y $h$ son siempre es simétricos.
...
%\end{definicion}
%
%Como $h$ es simétrica y $\intR h(\omega) d\omega = \sigma^{2}$, entonces 
%$H(0) = \nicefrac{\sigma^{2}}{2}$ y puede re-definirse
...
Como consecuencia del teorema de Wiener-Khinchin (\ref{t_wienerkhinchin}) 
se puede escribir
\begin{equation*}
