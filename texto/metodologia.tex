...
referida como SDF, mientras que los p valores. El p-valor para \textttt{T+I+R} es equivalente al 
...
se construyeron más graficos sobre la ubicación de épocas estacionarias en el tiempo. Estos
nuevos
gráficos, como el de la figura \ref{comp_VCR}, refuerzan la hipótesis de que los 
patrones son significativos. 
...
\caption{Cambio en el porcentaje de épocas esacionarias conforme el tamaño de ventana}
\label{cabeza_repoio}
\end{figure}
...
Cabe destacar que la aplicación \textit{per se} de la prueba fue realizada usando el software 
estadístico R \cite{R_citar}; en particular, se usó la implementación 
bajo la función \texttt{stationarity} del paquete \texttt{fractal} \cite{R_fractal}.

\subsection{Espectro de potencias}

Adicionalmente al estudio de estacionariedad, se calculó el espectro de potencia de banda ancha,
es decir la potencia total y relativa correspondiente a las ondas delta, theta, alfa, beta y gamma.
%
...
%Sin embargo, para poder asegurar heurísticamente 
...
\begin{equation}
R_{\text{lento}} = \frac{\text{potencia}_{\delta}+\text{potencia}_{\theta}}
