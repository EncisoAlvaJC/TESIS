...
for(ch in 1:n_canales){
  ...
  if(zoom){
    distr.t  = rep(0,mmm1*factor_escala)
    indice_aux = 1:mmm1
    for(k in 0:(factor_escala-1)){
      distr.t[indice_aux*factor_escala-k] = pv_t
    }
    distr.tir  = rep(0,mmm1*factor_escala)
    indice_aux = 1:mmm2
    for(k in 0:(factor_escala-1)){
  ...
  RES_T[ch,]   =   pv_t[1:length(RES_T[1,])]
  RES_TIR[ch,] = pv_tir[1:length(RES_T[1,])]
}
...
axis(3,labels=F,tick=T,at=c(0,n.epo)+.5)
#axis(4,at=c(0,n_canales)+.5,    labels=F,           las=2,tick=T)

axis(1,labels=F,tick=T,at=c(0,n.epo)+.5)
skip = seq(1,n.epo+1,by=paso/dur.min)+.5
