...
vectorr = rev(30*(2**(-5:2)))
lvv     = length(vectorr)

RES_T   = matrix(0,nrow=lvv,ncol=n_epocas)
RES_TIR = matrix(0,nrow=lvv,ncol=n_epocas)
...
for(i in 1:lvv){
  ...
  consta = dur_epoca/(30/32)
  for(j in 0:(consta-1)){
    RES_T[i,(1:mmm1)*consta-j]   =   pv_t[1:mmm1]
    RES_TIR[i,(1:mmm1)*consta-j] = pv_tir[1:mmm1]
  }
}
...
#################################################
# inicia grafico
colorgram(z=t(RES_T[rev(1:lvv),]),outlier='black',bty='n',axes=F,
          #xlab='Tiempo (hh:mm)',ylab='',
          #xlab='Num. de epoca',
          xlab='',
          #ylab='',
          ylab=ch_actual,las=2,cex.lab=2,font.lab=2,
          colFn=grayscale,
...
axis(2,at=1:lvv,labels=rev(vectorr),las=2,tick=F,cex.axis=.8)
axis(2,at=0:lvv+.5,    labels=F,           las=2,tick=T)
axis(4,at=c(0,lvv)+.5,    labels=F,           las=2,tick=T)
