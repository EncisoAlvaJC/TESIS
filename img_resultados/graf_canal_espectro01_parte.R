...
RES = matrix(0,nrow=6,ncol=n_epocas)
...
# }
# # organizacion de los datos en una matriz
# RES[7,] = pv_t[1:n_epocas]
# 
# mmin = quantile(as.numeric(RES[7,]),.1)
# mmax = quantile(as.numeric(RES[7,]),.9)
# RES[7,] = pmax(RES[7,],mmin)
# RES[7,] = pmin(RES[7,],mmax)
# mmin = quantile(as.numeric(RES[7,]),.1)
# mmax = quantile(as.numeric(RES[7,]),.9)
# RES[7,] = pmax(RES[7,],mmin)
# RES[7,] = pmin(RES[7,],mmax)
...
axis(2,at=1:6,labels=rev(c(banda.n[1:5],'P. total')),las=2,tick=F,cex.axis=.8)
axis(2,at=0:6+.5,    labels=F,           las=2,tick=T)
