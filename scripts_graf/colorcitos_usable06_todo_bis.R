...
EST.graf = ggplot(RES.algo,aes(x=Indice,y=Canal_var,fill=(Estacionario))) +
  geom_raster() +
  xlab(NULL) + ylab(NULL) +
  theme_bw() +
  scale_x_datetime(expand=c(0,0),breaks = NULL)+
...
the.graf = ggarrange(EST.graf,MOR.graf,
          ...
          heights = c(.35,.55,.1),legend = 'bottom')

ggsave(filename = paste0(nombre,'_EOG_EMG_combinado.png'),
