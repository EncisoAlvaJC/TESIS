...
RES.d = matrix(nrow=n_canales,ncol=n_epocas)
RES.t = matrix(nrow=n_canales,ncol=n_epocas)
RES.a = matrix(nrow=n_canales,ncol=n_epocas)
RES.b = matrix(nrow=n_canales,ncol=n_epocas)
...
for(ch in 1:n_canales){
  ...
  # organizacion de los datos en una matriz
  RES.b[ch,] = pv_t[1:n_epocas]
}
# fin ciclo que recorre canales
#################################################

RES = (RES.d+RES.t)/(RES.a+RES.b)
...
# grafico principal
  ...
  colorgram(z=t(RES[rev(1:n_canales),]),outlier='white',bty='n',axes=F,
            ...
            xlab='Num. de epoca',
            #ylab='',
            ylab='Enlentecimiento',las=2,cex.lab=2,font.lab=2,
            #colFn=grayscale,
