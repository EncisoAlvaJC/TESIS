...
#for(expon in (-5):(2)){
  ...
  setwd(dir_actual)
  source('colorcitos_usable07_parte.R')
}
...
EST.graf = 
  ...
  scale_x_datetime(expand=c(0,0),breaks = NULL)+
  scale_y_discrete(expand=c(0,0),
                   limits=rev(levels(RES.collect$Canal_var))) +
...
the.graf = ggarrange(EST.graf,MOR.graf,
          ncol=1,nrow=2,align = 'v',common.legend = TRUE,
          heights = c(.875,.125),legend = 'bottom')

plot(the.graf)

ggsave(filename = paste0(etiqueta,'_patrones_2.png'),
       path = dir_graf,units='cm',dpi=600,width=15,height=16,
       scale=1.4)
