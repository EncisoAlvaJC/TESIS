...
Cabe comentar  sobre la existencia de procesos que son fuertemente 
estacionarios pero que no son estacionarios de ning\'un orden: 
%dado que la primera 
...
por ejemplo,
un proceso de variables aleatorias independientes 
con distribuci\'on de Cauchy\footnote{Una variable aleatoria tiene distribuci\'on
de Cauchy si su funci\'on de probabilidad acumulada es de la forma
$\displaystyle F(x) = \frac{1}{\pi} \int_{-\infty}^{x} \frac{1}{1+y^{2}} dy$}.
Una condici\'on suficiente para que un proceso fuertemente estacionario sea estacionario de
orden $m$ es que tenga sus primeros $m$ momentos bien definidos.
Con respecto a las se\~nales registradas en el EEG, entendidas como procesos estoc\'asticos, 
se espera que tengan
%se suficientemente regulares 
%como para 
(cuando menos) segundos momentos bien definidos;
m\'as adelante se presentan argumentos, desde una interpretaci\'on f\'isica, sobre por qu\'e se 
espera que ocurra lo anterior.
%esta misma caracterizaci\'on bien puede fallar en el caso de
%se\~nales registrados y ajenas al EEG (ruido).
%
...
%Conviene exhibir (cuando menos) un tipo de regularidad que,
%%sirva para distinguir los patrones 
%%buscados de errores de medici\'on e interferencias.
%se espera, presenten las se\~nales investigadas pero no el ruido.

A continuaci\'on (definici\'on \ref{cont_est}) se presenta una segunda forma de regularidad que, se 
espera, exhiben las se\~nales registradas en el EEG.
...
Una forma natural de pensar en 
la definici\'on \ref{cont_est} 
es que, 
%en promedio,
...
si $\abso{t-t_0}$ es muy peque\~no, entonces $X(t)$ y $X(t_0)$ difieren muy poco entre s\'i.
Es destacable que las realizaciones de un proceso que es estoc\'asticamente continuo en un intervalo, 
solamente son continuas (en el sentido cl\'asico) casi en 
todas partes \footnote{Una propiedad se cumple \textbf{casi en todas partes} si se 
cumple en un conjunto cuyocomplemento tiene medida cero} en ese intervalo.
%De manera m\'as general, cabe mencionar un teorema que permite tipificar de manera m\'as
%adecuada esta clase de procesos.
En el teorema \ref{cont_est} se relacionan los proceso d\'ebilmente estacionarios con la continuidad
estoc\'astica
...
estoc\'asticamente continuos;
el proceso de Wiener, en contraparte, es el arquetipo de un proceso estoc\'asticamente continuo.
...
%As\'imismo existe una teor\'ia matem\'atica bien 
...
%entonces la siguiente igualdad se cumple casi en todas partes\footnote{Una propiedad se dice es 
%satisfecha \textbf{casi en todas partes} si se cumple en un conjunto cuyo complemento tiene 
%medidad cero; en este caso particular, se toman los reales con la medida usual} 
%\begin{equation*}
...
%no se discutir\'an detalles sobre cu\'ando est\'an bien definidas, o la convergencia de 
%las sumas indicadas,
%siempre que se limite su uso a funciones peri\'odicas acotadas absolutamente sumables 
%(en un intervalo hasdask), ignorando el comportamiento de la serie en puntos de discontinuidad.
...
%Una vez mencionada la transformada de Fourier como una funci\'on de un espacio de funciones a 
%un espacio de sucesiones
...
%separadamente para cada se\~nal base. 
...
le suele referir como \textbf{espectro de potencia}.
%\footnote{Dada esta discusi\'on, conviene 
