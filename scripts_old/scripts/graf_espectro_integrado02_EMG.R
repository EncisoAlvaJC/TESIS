...
dir_res_mid  = 'C:/Users/EQUIPO 1/Desktop/julio/espectro_integrado_170926/'
...
RES = matrix(nrow=5,ncol=n_epocas)
...
par(oma=c(0,0,0,0),
    mar=c(.5,2.6,0,3))
colorgram(z=t(RES[rev(1:5),]),outlier='white',bty='n',axes=F,
...
axis(2,at=1:5,labels=rev(banda.n[1:5]),las=2,tick=F)
axis(2,at=0:5+.5,    labels=F,           las=2,tick=T)
axis(3,labels=F,tick=T,at=c(0,n_epocas)+.5)
axis(4,at=c(0,5)+.5,    labels=F,las=2,tick=T)
