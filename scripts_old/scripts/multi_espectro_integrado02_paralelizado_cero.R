...
if(zoom){
  confirma_zoom = FALSE
  if(unidad_par_t == 'tiempo'){
    min_t  = min_hms[1]*60*60 + min_hms[2]*60 + min_hms[3] -ini_t - .5
    max_t  = max_hms[1]*60*60 + max_hms[2]*60 + max_hms[3] -ini_t + .5
    min_e  = floor((min_t+ini_t)/dur_epoca -ini_epo)
    max_e  = ceiling((max_t+ini_t)/dur_epoca -ini_epo)
    #min_e  = floor(min_t*ventana_reg)
    #max_e  = max_t*ventana_reg
...
#for(ch in (1:9)){
  ...
  pesca = foreach(i = 0:(max_epoca-2),
    ...
    return(c(i,i_banda..,i_banda.d,i_banda.t,i_banda.a,i_banda.b,
      ...
      i_dfa_r,i_exp.exp,i_exp.co,VARR))
    }
    return(c(i,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,var(tmp)))
  }
  ...
  vec.var = pesca[order(indices),13]
