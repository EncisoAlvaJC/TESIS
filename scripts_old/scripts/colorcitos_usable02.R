...
for(ch in 1:n_canales){
  ...
  nom_arch  = paste0('EST_',nombre,'_',ch_actual,'_TIR.txt')
  pv_tir    = scan(nom_arch)
  #pv_tir    = read.csv(nom_arch)
  #pv_tir    = as.numeric(t(pv_t[2]))
  #pv_tir    = as.numeric(t(pv_t))
  ...
  if(fr_muestreo==200){
    ...
    pv_tir2     = rep(0,3*length(pv_tir))
    for(i in 1:length(pv_tir)){
      pv_tir2[3*i-2] = pv_tir[i]
      pv_tir2[3*i-1] = pv_tir[i]
      pv_tir2[3*i  ] = pv_tir[i]
    }
    pv_tir = pv_tir2
  }
