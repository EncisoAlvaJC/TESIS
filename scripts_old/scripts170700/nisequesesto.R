...
#dir_res     = paste0(getwd(),'/graficos')

nombre      = 'MJNNVIGILOS'
etiqueta    = 'MJNN'
dir_datos   = paste0(getwd(),'/MJNNVIGILOScCanal')
dir_res     = paste0(getwd(),'/poster_170718')
...
if(grabar){
  ...
  png(paste0(etiqueta,'_',
             ...
             '.png'),units='cm',res=300,width=36*1,height=9*1)
}
...
#################################################
# graficacion
# par(bg = 'NA', mar = c(3,1,2,1))
# plot(c(1,max(ind_pt)),
#      type='o',col='white',
#      ylim=c(0,n_canales*escala_mv),
#      xlim=c(1,max(ind_pt)),
...
#   correccion = (n_canales-ch)*escala_mv+escala_mv/2 - mean(DATA)
#   lines(ind_pt,DATA[ind_pt] + correccion,
...
#          mgp=c(0,-1.5,-2.5))
#   }else{
#     axis(4,at=(n_canales-ch)*escala_mv+escala_mv/2,
#          labels=ch_actual,las=1,col.axis=vec_colores[ch],
#          mgp=c(0,-1.5,-2.5))
#   }
# }
...
# fin guardado de los graficos
#################################################

setwd(dir_actual)
   
   te = seq(1,length(DATA))/512
   
dev.off()

par(te,mfrow=c(2,1))
par(mar = c(3,3,1,1))
plot(te,DATA,type='l',ylab='mV',xlab='Tiempo (s)',bty='n',
     mgp=c(2,1,0))

sq = 4000:(4000+512*8)

lines(te[sq],DATA[sq],type='l',col='red')

ef = Mod(fft(DATA[sq]))
ef = ef[1:(length(ef)/2)]
de = seq(1:length(ef))
plot(de[1:1000],log(ef[1:1000]),type='l',ylab='',xlab='Frecuencia (Hz)',bty='n',
     mgp=c(2,1,0))
