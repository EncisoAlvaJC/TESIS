...
dir_res_mid  = paste0(getwd(),'/colores')
nombre       = 'CLMN10SUE'
etiqueta     = 'CLMN'
#nombre       = 'MJNNVIGILOS'
#etiqueta     = 'MJH'
...
#reemplazar = F
#canales    = c('C3','C4','CZ','F3','F4','F7','F8','FP1','FP2','FZ')
...
for(ch in 1:n_canales){
  ...
  if(file.exists(paste0('COL_',nombre,'_',ch_actual,'.txt'))){
    correctos[ch] =TRUE
  }else{
...
ch        = 1
ch_actual = canales[ch]
nom_arch  = paste0('COL_',nombre,'_',ch_actual,'.txt')
#pv_t      = scan(nom_arch)
