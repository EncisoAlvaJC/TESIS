...
ind_t  = (0:n_puntos)*(salto/fr_muestreo) + ini_t
ind_hh = floor(ind_t/(60*60))
ind_mm = floor( (ind_t - ind_hh*60*60)/60 )
ind_ss = floor(  ind_t - ind_hh*60*60 - ind_mm*60 )
ind_ms = ind_t - ind_hh*60*60 - ind_mm*60 - ind_ss
txt_t  = character(n_puntos+1)
...
axis(1,at=ind_t*salto,labels=txt_t[ind_t])
axis(3,at=c(min(ind_t),max(ind_t))*salto,labels=F,
     labels=txt_t[c(min(ind_t),max(ind_t))])
