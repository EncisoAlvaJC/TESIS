...
#      22 : T6
#
multipsr <- function(archivo,w_dir,c_dir,r_dir,
                     frec,dur_epo){
  ...
  nom_ar = paste0(archivo,'.txt')
  
  # cargar datos
  setwd(w_dir)
  DATA = read.csv(nom_ar,sep = '\t')
  DATA = as.numeric(unlist(DATA[,2]))
  ...
  setwd(r_dir)
  write.csv(pv.t  , paste0('EST_',archivo,'_T.csv'  ))
  write.csv(pv.ir , paste0('EST_',archivo,'_IR.csv' ))
  write.csv(pv.tir, paste0('EST_',archivo,'_TIR.csv'))
}
