colorcitos <- function(sujeto, frecuenciasss, v.nombres, v.etiqueta, v.directorio, dir_res_mid, sizeEpoc){
  ###############################################################################
  dir_actual  = getwd()
  
  nombre      = v.nombres[sujeto]
  etiqueta    = v.etiqueta[sujeto]
  dir_graf = paste0(dir_res_mid,'Imagenes/')
  ...
  for(ch in 1:n_canales){
    ...
    if(file.exists(paste0('EST_',nombre,'_',ch_actual,'_T_', sizeEpoc, '.csv'))){
      correctos[ch] =TRUE
    }else{
  ...
  ch        = 1
  ch_actual = canales[ch]
  nom_arch  = paste0('EST_',nombre,'_',ch_actual,'_T_', sizeEpoc, '.csv')
  #pv_t      = scan(nom_arch)
