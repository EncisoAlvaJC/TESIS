...
dir_actual = 'C:/Users/EQUIPO 1/Desktop/julio/tesis_respaldo/TESIS/img_diagramas'
...
ggsave(file='espectrosA_v3.png',device='png',
       width=5,height=2,unit='in',scale=1.35,dpi=400)
...
i2 = ggplot(SS,aes(x=tt,y=xx,color=tipo1))+
  ...
  #ylab('Amplitud [mV]') +
  labs(y=expression(paste("Amplitude [", mu, "V]")))+ 
  labs(title='Subject: ---  |  Channel: ---  |  Epoch = 30 s')+
...
ggsave(file='espectrosB_v3.png',device='png',
       width=3.25,height=2,unit='in',scale=1.35,dpi=400)
...
i3 = ggplot(SS,aes(x=tt,y=xx,color=tipo2))+
  ...
  #ylab('Amplitud [mV]') +
  labs(y=expression(paste("Amplitude [", mu, "V]")))+ 
  labs(title='Subject: ---  |  Channel: ---  |  Epoch = 10 s')+
  scale_color_gradient(low=mora1,high=azul2)+
  scale_x_datetime(expand=c(0,0),labels=date_format("%M:%S"),
                   breaks = date_breaks("15 sec"))+
  theme_bw()+
...
ggsave(file='espectrosC_v3.png',device='png',
       width=3.25,height=2,unit='in',scale=1.35,dpi=400)
...
i4 = ggplot(MM,aes(x=tt,y=Relleno,fill=Val,colour='black',
              linetype='dotted'))+
  xlab('Time [mm:ss]') + ylab('Channel: ---') +
  labs(title='Subject: ---  |  Epoch = 30 s')+
  theme_bw() +
  scale_x_datetime(expand=c(0,0),labels=date_format("%M:%S"),
                   breaks = date_breaks("15 sec"))+
...
ggsave(file='espectrosD_v3.png',device='png',
       width=3.25,height=1.5,unit='in',scale=1.35,dpi=400)
...
ggsave(file='espectrosE_v3.png',device='png',
       width=3.25,height=1.5,unit='in',scale=1.35,dpi=400)
