...
autocovarianza, entonces la soluci\'on es evitar tales colas multiplic\'andolas por una funci\'on 
de pesos que decae r\'apidamente. En este sentido, se considerar\'an estimadores de la forma
\begin{equation*}
\est{h}(\omega) = \frac{1}{2\pi} \sum_{s = -(N-1)}^{N-1} 
\lambda(s) \aste{R}(s) e^{i \omega t}
\label{ventaneando}
\end{equation*}
donde $\lambda$ es la funci\'on de pesos, referida como \textbf{ventana de retrasos}. Para estudiar 
las propiedades este tipo de estimadores, conviene reescribirlos en funci\'on del periodograma
