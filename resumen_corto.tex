...
%El objeto de estudio del presente trabajo consiste en 
registros de actividad cerebral durante el 
sue\~no (polisomnograf\'ia, PSG), modelados como procesos estoc\'asticos a tiempo continuo. 
...
que en casos at\'ipicos, como el del DC, podr\'ia no cumplirse. 
...
est\'a relacionado con la 'estructura fractal' de una serie de tiempo
(realizaci\'on de un proceso estoc\'astico)
de modo que un mayor exponente est\'a asociado con se\~nales 
cuya funci\'on de autocorrelaci\'on 
decrece m\'as lentamente \cite{Bryce12}.
...
%Cabe mencionar que el presente trabajo resulta de una colaboraci\'on con el departamento de
%Gerontolog\'ia, dependiente del Instituto de Ciencias de la Salud (ICSA) de la Universidad 
%Aut\'onoma del Estado de Hidalgo. Parte de esta colaboraci\'on incluye el acceso a los registros de
%PSG.
...
en M\'exico habitan cerca de 12,436,321 adultos mayores, lo que representa un 10.4 \%  de la 
poblaci\'on.
%3,889,755
%2,987,316
%2,207,466
...
esta descripci\'on se basa principalmente en libro \textit{Spectral Analysis and Time Series} de
M. B. Priestley \cite{Priestley81}.
...
%estoc\'astica.
Como ejemplos, un proceso ruido blanco (definici\'on \ref{r_blanco}) no
es estoc\'asticamente continuo, mientras que un proceso de
Wiener (definici\'on \ref{r_wiener}) s\'i lo es.
...
$e^{i q t}$}; suponiendo previamente que las se\~nales se suman y multiplican por escalares de la manera 
usual. 
Esta afirmaci\'on es equivalente a que el conjunto de funciones con una serie de Fourier 
...
disipada
(definici\'on \ref{energia}) en conjunto con el el teorema \ref{parseval_serie}, ya que permite formular la 
siguiente interpretaci\'on: la energ\'ia disipada por una se\~nal peri\'odica puede verse como la suma 
de la energ\'ia disipada por sus componentes en la base de Fourier (usualmente referidos como
...
Sea $P_T$ el espacio de las funciones peri\'odicas con periodo $2T$ que tienen una serie de Fourier 
bien definida. Se le llama espectro de potencias a la 
funci\'on $h: P_T \times \mathbb{R} \mapsto \mathbb{R}$ definida como
\begin{equation*}
h_k(\omega) = 
\begin{cases}
...
sobre funciones absolutamente sumables; se pueden extender
sencillamente la transformada de Fourier y el espectro de potencias para esta funci\'on.
%, conservando interpretaciones f\'isicas similares.

\begin{defn}[Integral de Fourier]
Sea $k$ una funci\'on tal que $\intR \abso{k(t)} dt < \infty$. 
Se le llamar\'a 'integral de Fourier para la funci\'on $k$' a la funci\'on 
...
La integral de Fourier  se encuentra bien definida para cualquier $\omega \in \mathbb{R}$,
e incluso se puede probar que es una funci\'on continua y que converge a 0 en $\pm \infty$.
Sin embargo,
por simplicidad en la explicaci\'on se omiten los detalles sobre la interpretaci\'on f\'isica y
anal\'itica de la integral de Fourier; en su lugar se usar\'a como motivaci\'on para la
transformada de Fourier-Stieltjes.

\begin{defn}[Transformada de Fourier-Stieltjes]
Se dice que una funci\'on real $A$ es la transformada de
Fourier-Stieltjes de una funci\'on $k$ si \'esta puede expresarse casi en todas partes como
\begin{equation*}
...
Es destacable en la definici\'on \ref{fourier_stieltjes} que si $F$ es derivable 
en todas partes, entonces $F\prima$ cumple el mismo papel que la integral de Fourier; por otro lado,
si $k$ es una funci\'on peri\'odica entonces $F$ toma una forma escalonada cuyos aumentos 
coinciden con la serie de Fourier para $k$. M\'as a\'un, existen funciones que no son ni peri\'odicas
ni absolutamente sumables pero poseen una transformada de Fourier-Stieltjes, como
$k(x)=\SEN{x}+\SEN{\sqrt{2}x}$ o $k(x)=\COS{x} + (1+x^{2})^{-1}$
...
distribuci\'on de probabilidad son de variaci\'on acotada, implica que una 
...
La forma m\'as natural de definir una transformada de Fourier para un proceso estoc\'astico es
a trav\'es de las realizaciones del mismo; sin embargo, tal idea implica manejar la posibilidad
de que tales realizaciones tengan una transformada de Fourier bien definida.
El enfoque que se aborda es considerar a $x(t)$, realizaci\'on arbitraria de un proceso 
d\'ebilmente estacionario $\{X(t)\}$, y aproximarlo con funciones con transformadas de
Fourier bien definidas.
%construir una sucesi\'on de funciones que posean serie de Fourier y que 
%convergen a realizaciones arbitrarias del proceso, $x(t)$; para cada $T>0$ se define la 
%restricci\'on $x_T$ como
As\'i entonces, para cada $T>0$ se define la funci\'on $x_T$ como
\begin{equation*}
x_T(t) = 
\begin{cases}
...
Posteriormente se define a $G_T$, una transformada de Fourier para $x_T$. Bien 
puede entenderse a $G_T$ como una transformada de Fourier para una funci\'on extendida 
peri\'odicamente, y de modo que el espacio entre frecuencias se vuelven infinitesimal; 
bien puede verse como la integral de Fourier, claramente bien definida, para una serie
convergente de funciones.

%de modo que las frecuencias se vuelven infinitesimales; o puede 
...
$\left| G_T(\omega) \right|^{2}$ como una funci\'on de distribuci\'on para la energ\'ia disipada,
hay un argumento f\'isico seg\'un el cual
$G_T$ diverge: durante un tiempo infinito, un sistema que maneja 'niveles constantes' 
de energ\'ia puede disipar una cantidad infinita de energ\'ia. 
Entonces, conviene promediar sobre el tama\~no de los intervalos (que son 
finitos excepto en el l\'imite) de manera que se pueda converger a una densidad de energ\'ia disipada.
...
La expresi\'on anterior se puede adaptar al proceso per se, como se muestra en la definici\'on 
\ref{SDF}.
...
Conviene mencionar que la convergencia de la SDF est\'a garantizada casi en todas partes, pero que
eso puede no ser suficiente para explicar completamente al proceso; por ejemplo, un proceso 
peri\'odico, con periodo $T$, tendr\'a una SDF que es cero excepto en los m\'ultiplos enteros de $T$.
Este fen\'omeno es similar al caso de variables aleatorias discretas, que no poseen una funci\'on de densidad
de probabilidad bien definida en todos sus puntos.
Conviene definir una veris\'on integrada de la SDF integral de la SDF ( definici\'on\ref{SDF_integrado}).
...
Siempre 
que $h$ est\'e bien definida puede decirse que $H\prima= h$, en cuyo caso se dir\'a que el proceso 
tiene un espectro puramente continuo; si, por el contrario, $H$ tiene una forma escalonada, se
dir\'a que el espectro es puramente discreto.
De manera completamente an\'aloga al caso en que la transformada de Fourier-Stieltjes puede
interpretarse como una medida;
m\'as a\'un, es positiva, no-decreciente y acotada (bajo condiciones que se discutir\'an m\'as adelante).
...
En este caso se muestran algunos teoremas respecto a la SDF que permitir\'an estimarla 
en el caso de proceso d\'ebilmente estacionarios.
...
Una consecuencia de los teoremas de Wiener-Khinchin y de Wold, de la que no se hab\'ia hablado
...
una descomposici\'on 
%de Lebesgue (definici\'on \ref{Lebesgue_decomp}) 
