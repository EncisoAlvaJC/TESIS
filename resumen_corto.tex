...
A grosso modo, la AAIC clasifica las etapas del sue\~no \cite{Hori01} con las siguientes 
caracter\'isticas:
...
Si la SDF, $h$, est\'a bien definida en todos sus puntos, entonces la funci\'on de espectro integrado
($H$) satisface que $H\prima= h$ y se dir\'a
que el proceso tiene un \textbf{espectro puramente continuo}; si $H$ tiene una forma escalonada, con
escalones rectos, se dir\'a que es un \textbf{espectro puramente discreto}.
%De manera completamente an\'aloga al caso en que la transformada de Fourier-Stieltjes puede
%interpretarse como una medida; m\'as a\'un, es positiva y no-decreciente.
Como es de esperarse, cada tipo de proceso tiene caracter\'istica diferentes y se puede 
estudiar mejor con herramientas diferentes; para el caso de procesos con un espectro 
mixto (ninguno de los anteriores), se exhiben herramientas que los reducen a estos casos 'puros'.
...
Una consecuencia de los teoremas de Wiener-Khinchin y de Wold
es poder caracterizar a los procesos d\'ebilmente estacionarios 
usando directamente la SDF, y que ser\'a usada en su estimaci\'on.
...
Una raz\'on muy particular para usar la representaci\'on de Wold-Cram\'er es el manejo de procesos
'filtrados' del tipo $Y(t) = \intR g(u) X(t-u) du$.
Hist\'oricamente, los filtros se originan en teor\'ia de circuitos como circuitos f\'isicos con ciertas 
caracter\'isticas (que no ser\'an discutidas en este texto); destacan dentro de la estimaci\'on 
anal\'ogica de la SDF y en la 'cancelaci\'on' anal\'ogica de componentes de frecuencia (ver m\'as
adelante).
%
Por otro lado, los filtros permiten interpretar a procesos MA, AR y ARMA, como versiones
filtradas de procesos ruido blanco; m\'as a\'un, si se permite que los procesos filtrados tengan la
forma\footnote{La integral est\'a definida en el sentido de Stieltjes} $Y(t) = \intR X(t-u) dG(u)$ 
entonces es posible manejar operadores discretos, como $B[Y](t) = Y(t)-Y(t-1)$.
%
Los filtros, aplicados a proceso d\'ebilmente estacionarios, afectan su SDF de manera
'simple' (teorema \ref{filtro}).  
...
En el teorema \ref{filtro}, la funci\'on $g$ es referida como 
\textbf{funci\'on de respuesta}; tal nombre tiene sentido si $\{X(t)\}$ no fuera un proceso, sino
un 'impulso unitario' (una funci\'on tipo \dirac).
...
La funci\'on $\Gamma$ es referida como \textbf{funci\'on de transferencia}, nombre motivado de manera similar
al considerar a $\{X(t)\}$ una 'se\~nal' senoidal ($X(t) = e^{i \omega t}$);
...
%forma similar pero con otra frecuencia. 
la conexi\'on se vuelve m\'as clara 
si se interpreta esta segunda se\~nal como funci\'on tipo 
...
conjunto que no contenga a $x$.} en el espacio de las frecuencias.
Respecto a la 'cancelaci\'on' de frecuencias basta decir que, en teor\'ia, 
se puede construir a $g$ de modo que $\Gamma$ sea 0 fuera de un intervalo cerrado y 1 dentro del mismo;
este efecto es referido como \textbf{filtro de banda}.
...
Esta t\'ecnica fue presentada por Priestley y Subba Rao en 1965 \cite{Priestley69}, y consiste en 
estimar el espectro del proceso 'localmente en muchos lugares' para luego revisar si se puede 
rechazar o no (como prueba de hip\'otesis) el que sean estad\'sticamente constantes en el tiempo;
la explicaci\'on presente es m\'as bien somera, los detalles importantes (la existencia del proceso
evolutivo para cierto tipo de procesos, las propiedades de los estimadores usados) son tomadas de
\cite{Priestley65} y \cite{Priestley66}.

Para usar este m\'etodo, se supone que los datos pueden entenderse como una cantidad finita de 
...
La cantidad de condiciones puede parecer excesiva, pero es remarcable que no se ha supuesto que
el proceso sea estacionario y que previamente se presentaron motivos por los que los registros
de PSG son observaciones de procesos a tiempo continuo, estoc\'asticamente continuos, con valor 
esperado y varianzas finitas en todo momento; las propiedades de tener media cero y espectro 
puramente continuo ser\'an 'forzadas' num\'ericamente (ver subsecci\'on siguiente).

Respecto a la estimaci\'on local del espectro, se usa el estimador de doble ventana, t\'ecnica 
introducida por Priestley y Subba Rao \cite{Priestley69}, la cual requiere que se proporcionen 
a priori dos funciones $w_\tau$ y $g$ con ciertas caracter\'isticas; deber\'ian 
funcionar, respectivamente, como ventana de retrasos y como filtro lineal.
%
...
a la SDF en el tiempo dado; m\'as a\'un, $U$ sirve 
