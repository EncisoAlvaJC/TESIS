...
for(ch in 1:n.canales){
  ...
  if(FALSE){
    A = ezANOVA(data=tmp,dv=Hurst,wid=Sujeto_n,
                within=MORn,between=.(Grupo,Etapa),
  ...
  if(FALSE){
    ...
    Anova(aov)
    Anova(aov,type='II',idata=Sujeto_n,
          univariate=T,multivariate=F,style='long')
    
    aov2  = aov(Hurst ~ (Grupo) + (Etapa) + MORn +
                  (Grupo:Etapa) + (Grupo:MORn) + (Etapa:MORn) +
                  (Grupo:Etapa:MORn),
                data=tmp)
  }
  ...
  ezPlot(data=tmp,dv=Hurst,wid=Sujeto_n,
         x=.(Etapa),
         within=.(Etapa),between=.(Grupo),
         within_full = .(MORn,Etapa),
         #observed=.(Etapa,Grupo,MORn),
