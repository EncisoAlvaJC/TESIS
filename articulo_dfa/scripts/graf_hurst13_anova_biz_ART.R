...
if(FALSE){
  ...
  test = is.element(Hurst.todo$Sujeto_n,c('CLO'))
...
for(ch in 1:n.canales){
  ...
  tab.multi[ch,] = c(KS1$statistic,KS1$p.value,
                  KS2$statistic,KS2$p.value,
                  KS3$statistic,KS3$p.value,
                  KS4$statistic,KS4$p.value)
