...
Hurst.todo = melt(raw,id=c('Sujeto','Grupo','Edad','MMSE','Neuropsi',
                           'MORn','Epoca'))
colnames(Hurst.todo) = c('Sujeto','Grupo','Edad','MMSE','Neuropsi',
                         'MORn','Epoca','Canal_var','Hurst')
Hurst.todo$Canal_var = as.numeric(Hurst.todo$Canal_var)

Hurst.todo = Hurst.todo[!is.na(Hurst.todo$Hurst),]
Hurst.todo$Sujeto_n = factor(Hurst.todo$Sujeto,labels = info$Nombre[1:10])


Hurst.promedio = c()
for(suj in 1:10){
  tmp       = Hurst.todo[grep(info$Nombre[suj],Hurst.todo$Sujeto_n),]
  promedios = aggregate(tmp,by=list(tmp$Canal_var),mean)
  Hurst.promedio = rbind(Hurst.promedio,promedios)
}

Hurst.promedio$Sujeto_n = info$Nombre[Hurst.promedio$Sujeto]
...
Hurst.todo$Canal_var     = factor(Hurst.todo$Canal_var,
                                  labels=kanales$Etiqueta)
Hurst.todo$Grupo         = factor(Hurst.todo$Grupo,
                                  labels=c('CTRL','PMCI'))
Hurst.promedio$Canal_var = factor(Hurst.promedio$Canal_var,
                                  labels=kanales$Etiqueta)
Hurst.promedio$Grupo     = factor(Hurst.promedio$Grupo,
                                  labels=c('CTRL','PMCI'))
