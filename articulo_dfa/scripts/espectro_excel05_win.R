...
promedios = summarySE(RES.todo,measurevar='Delta',
...
for(banda.actual in 1:n.bandas){
  ...
  ggplot(promedios,aes(x=Canal,y=Delta,fill=Etapa)) +
    labs(title=etiqueta,
...
# ggbarplot(RES.todo,x='Canal',y='Delta',fill='Etapa',add='mean_se',
#           position = position_dodge(.75),error.plot='errorbar') + 
#   labs(title=etiqueta,
#        subtitle=paste('Comparación MOR vs NMOR para la banda',bandas$Banda[banda.actual]))+
#   theme_pubr() +
#   theme(legend.position='bottom')+
#   stat_compare_means(aes(group=Etapa),
#                      label='p.signif',method='wilcox.test',hide.ns=T)+
#   xlab(NULL) + ylab(NULL) +
#   rotate_x_text(angle = 45)
