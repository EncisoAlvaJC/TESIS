...
min_hms_0 = c(h_ini[sujeto],m_ini[sujeto],s_ini[sujeto])
max_hms_0 = c(h_fin[sujeto],m_fin[sujeto],s_fin[sujeto])

maxi.chunk = hms2t(max_hms_0-min_hms_0)/dur.chunk
...
for(cual in 1:maxi.chunk){
  
  min_hms = min_hms_0 + c(0,0,(cual-1)*dur.chunk)
  max_hms = min_hms + c(0,0,30)
  
  min_hms = corregir.hms(min_hms)
  max_hms = corregir.hms(max_hms)
...
if(grabar){
  ...
  apoca = toString(hms2t(min_hms)/dur.chunk)
...
lines(fr_muestreo*(.5*sep_lineas+c(0,2)),
      escala_mv*c(1,1),
      type='l',lwd=1.5)
