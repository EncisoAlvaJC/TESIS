...
ggplot(RES.NMOR,aes(x=Neuropsi,y=Proporcion ,
                   shape=Grupo,color=Grupo))+
  ylab('Stationary epochs [%]') +
  labs(shape=NULL,color=NULL) +
  facet_wrap(~Canal_var,ncol=5) +
...
ggplot(RES.MOR,aes(x=Edad,y=Proporcion ,
                   shape=Grupo,color=Grupo))+
  ylab('Stationary epochs [%]') +
  labs(shape=NULL,color=NULL) +
  facet_wrap(~Canal_var,ncol=5) +
  theme_classic2() +
  geom_smooth(method=lm,
              mapping=aes(x=Edad,y=Proporcion ),
              inherit.aes=F,
              se=F,color='black') +
  scale_colour_discrete(guide = FALSE) +
  rotate_x_text(angle = 45)+
...
ggplot(RES.NMOR,aes(x=Edad,y=Proporcion ,
                   shape=Grupo,color=Grupo))+
  ylab('Stationary epochs [%]') +
  labs(shape=NULL,color=NULL) +
  facet_wrap(~Canal_var,ncol=5) +
...
# MOR
comparaciones     = matrix(0,nrow=n.canales,ncol=3)
comparaciones[,1] = kanales$Etiqueta
colnames(comparaciones) = c('Canal','p','W')
for(ch in 1:n.canales){
  ch.actual = kanales$Etiqueta[ch]
  ...
  comparaciones[ch,'W']  = k$statistic
  #comparaciones[ch,'m1'] = k$estimate[1]
  #comparaciones[ch,'m2'] = k$estimate[2]
}

A = ggplot(promedios.MOR,aes(x=Canal_var,y=Proporcion,fill=Grupo)) +
  theme_classic2() +
  labs(fill=NULL) +
...
#ggsave(filename='/comparacion_whole_stat_mor.png',path=dir_graf,
...
ggplot(promedios.NMOR,aes(x=Canal_var,y=Proporcion,fill=Grupo)) +
  ...
  rotate_x_text(angle = 45)
ggarrange(A,abels='B')
#ggsave(filename='/comparacion_whole_stat_nmor.png',path=dir_graf,
#       device='png',units='cm',width=8,height=4,dpi=400,scale=2)
...
# 
# ggsave(filename='/comparacion_whole_stat_signif_2.png',path=dir_graf,
...
for(ch in 1:22){
  ...
  aov   = aov(Proporcion ~ factor(Grupo) + factor(Etapa),
              data=tmp)
  #aov = kruskal.test(Proporcion ~ factor(Grupo) + factor(Etapa),
  #                   data=tmp)
  #aov = friedman.test(Proporcion ~ Grupo | Etapa,
  #                    data=tmp)
  #aov
  print(aov)
  k = summary(aov)
  ...
  p = ggplot(tmp.m,aes(x=Etapa,y=Proporcion,linetype=Grupo))+
    ...
    geom_errorbar(aes(ymin=Proporcion-sd,ymax=Proporcion+sd))+
    geom_point()
  print(p)
...
for(ch in 1:22){
  ...
  aov   = raov(Proporcion ~ (Grupo) + (Etapa) + (Grupo:Etapa),
              data=tmp)
  #aov = kruskal.test(Proporcion ~ factor(Grupo) + factor(Etapa),
  #                   data=tmp)
  #aov = friedman.test(Proporcion ~ Sujeto | Grupo,
  #                    data=tmp)
  
  model = glm(Proporcion ~ Grupo + Etapa + 
                Grupo:Etapa,
              data = tmp)
  #aov
  print(aov)
