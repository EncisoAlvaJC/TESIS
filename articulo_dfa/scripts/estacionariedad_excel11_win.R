...
for(ch in 1:n.canales){
  ...
  RES.t[ch,] = 1-pmin(1*(datos.t<.05),1*(datos.tir<.05))
...
RES.suma$Proporcion = 100*RES.suma$Epocas_estacionarias/RES.suma$Epocas
...
print(
ggplot(RES.todo,aes(x=Indice/factor.extra,y=Canal,fill=Estacionario)) + 
