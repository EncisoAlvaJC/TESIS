...
Hurst.todo$MORn[Hurst.todo$Etapa!='NREM'] = Hurst.todo$MORn[Hurst.todo$Etapa!='NREM'] + 10

Hurst.todo$Hurst[Hurst.todo$Canal_var=='EMG'] = 1.5

promedios.todo$xx = kanales$x[promedios.todo$Canal_var]
promedios.todo$yy = kanales$y[promedios.todo$Canal_var]

ggplot(Hurst.todo,aes(x=MORn,y=Hurst,color=Grupo))+
  theme_classic2() +
  ylab('Hurst exponent')+ xlab('Sleep stage')+
  geom_line(aes(group=Sujeto_n))+
  #geom_abline(y=10.5) +
