...
# para el sujeto VCR
indice.epocas = 712:(712+20*factor.extra-1) - delay
#diez = 10*factor.extra
diez = 0
...
RES.todo$Tiempo = (RES.todo$Indice/factor.extra) -1 + indice.epocas[1]

print(
  ggplot(RES.todo,aes(x=Tiempo,y=Canal,fill=Estacionario)) + 
    #scale_x_continuous(breaks = pretty((indice.epocas-indice.epocas[1])*30/factor.extra, n = 20),
    #                   expand=c(0,0)) +
    scale_fill_manual(values = c('white','black'))+
    xlab(NULL) + ylab(NULL) +
    ...
    #      legend.justification=c(1,0))+
    #scale_x_continuous(expand=c(0,0),breaks = 2*(0:5))
    scale_x_continuous(expand=c(0,0),breaks = pretty(RES.todo$Tiempo))
)
